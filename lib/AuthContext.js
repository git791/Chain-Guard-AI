...
export function AuthProvider({ children }) {
  ...
  const [profile, setProfile] = useState({ name: "Aryan Kumar", role: "Logistics Manager" });

  const updateProfile = (newProfileInfo) => {
    setProfile((prev) => ({ ...prev, ...newProfileInfo }));
  };
  ...
  return (
    <AuthContext.Provider value={{ user, login, logout, profile, updateProfile, loading }}>
      {children}
